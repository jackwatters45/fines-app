...
> **Note:** Using Effect Atom with AtomRpc for end-to-end type-safe client/server communication. Backup option: server functions with `Effect.promise()` wrapper. This diverges from lax-db (which has Atom set up but uses TanStack Query). Goal is to validate Effect Atom in production.
...
const playersAtom = Atom.make(
  Effect.promise(() => getPlayersServerFn())
).pipe(Atom.withReactivity(["players"]));
