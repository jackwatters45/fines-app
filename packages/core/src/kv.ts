...
const make = (kv: KVNamespace) => ({
  get: Effect.fn('KV.get')(function* (key: string) {
    return yield* Effect.tryPromise(() => kv.get(key)).pipe(
      ...
      Effect.mapError((cause) => new KVError({ message: `Failed to get key: ${key}`, cause })),
    );
  }),

  set: Effect.fn('KV.set')(function* (key: string, value: string, options?: KVNamespacePutOptions) {
    return yield* Effect.tryPromise(() => kv.put(key, value, options)).pipe(
      Effect.mapError((cause) => new KVError({ message: `Failed to set key: ${key}`, cause })),
      Effect.asVoid,
    );
  }),
  ...
  list: Effect.fn('KV.list')(function* (options?: KVNamespaceListOptions) {
    return yield* Effect.tryPromise(() => kv.list(options)).pipe(
      Effect.mapError((cause) => new KVError({ message: 'Failed to list keys', cause })),
    );
  }),
});

export class KV extends Context.Tag('@fines-app/KV')<KV, ReturnType<typeof make>>() {
